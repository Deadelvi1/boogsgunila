	...
	<form method="POST" action="{{ route('gedung.update', $item->id) }}">
		...
		<div class="space-y-4">
			...
			<div>
				...
				<input type="number" name="kapasitas" value="{{ old('kapasitas', $item->kapasitas) }}" min="0"
					class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
					placeholder="Jumlah orang">
			</div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Harga (Rp)</label>
                <input type="number" name="harga" value="{{ old('harga', $item->harga ?? 0) }}" min="0" step="0.01"
                    class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
			...
			<div>
				...
				<textarea name="deskripsi" rows="4"
					class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
					placeholder="Deskripsi gedung...">{{ old('deskripsi', $item->deskripsi) }}</textarea>
			</div>
